...
func _ready() -> void:
	scenes["hallway"]["button"] = $hallWay_Door/Btn_salir
	player.limit_left = -215
	player.limit_right = 1060

	if GlobalTransition.player_position_azotea != Vector2():
		player.position = GlobalTransition.player_position_azotea
...
func _on_btn_entrar_pressed(door_name: String) -> void:
	if scenes.has(door_name):
		...
		GlobalTransition.player_position_azotea = player.position
